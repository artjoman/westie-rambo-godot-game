...
Before exporting, run the regression suite instead of writing a one-off scratch script: `godot --headless --path . tests/regression_test.tscn`. It boots every level and the main menu, verifies music generation/playback (including the loop-guard padding that fixed a real on-device SIGSEGV), checks the touch controls' sizing and 8-directional joystick input, and exercises both cheat codes — printing a PASS/FAIL summary and exiting `0` only if everything passed. Extend it with a new `_check_*` function (see `tests/regression_test.gd`'s header comment) rather than spinning up a throwaway `scratch_*.gd`/`.tscn`.

After every successful build/release — the regression suite exits `0`, and an `--export-debug` or `--export-release` run completes without errors, and (when a device is available) installs and launches cleanly — commit the working tree and push to `origin/main`. This is standing authorization to push after a verified successful build; no need to ask each time. Write a real commit message describing what changed, not a generic "build" message. If the build fails or verification turns up a crash/regression, fix it first — don't commit a known-broken build.
